function [c, ceq] = nlcon_keyboard(x)
    ...
    c(1) = (sqrt(Rcb_bx_k^2 + Rcb_bz_k^2) / Acb_b_k) - (sigma_b / K);
	...
	c(2) = sqrt(Jcb_bz_k^2 + Jcb_bx_k^2) - (0.001 / K);
    c(3) = Jcb_az_k - (0.001 / K);
	...
	c(6) = (sqrt(Ex^2 + Ez^2) / pi*r_b^2) - (sigma_b / K);
	...
	c(10) = sqrt(Uaf_z^2 + Uaf_x^2) - (0.001 / K);
